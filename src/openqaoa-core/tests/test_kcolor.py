...
class TestKColor(unittest.TestCase):
    ...
    def test_kcolor_random_problem(self):
        ...
        k = 4
        np.random.seed(seed)
        kcolor_manual_prob = KColor.random_instance(n_nodes=n_nodes,
                                                    edge_probability=edge_probability,
                                                    k=k,
                                                    seed=seed)
        kcolor_random_prob = KColor(nx.generators.random_graphs.fast_gnp_random_graph(n=n_nodes, p=edge_probability, seed=seed), k)
        ...
        self.assertEqual(kcolor_manual_prob.docplex_model.export_to_string(), kcolor_random_prob.docplex_model.export_to_string())
    ...
    def test_kcolor_classical_sol(self):
        ...
        np.random.seed(seed)
        kcolor_sol = KColor.random_instance(n_nodes=n_nodes, 
                                            edge_probability = edge_probability,
                                            k=k,
                                            seed=seed).classical_solution(string=True)
    ...
    def test_kcolor_plot(self):
        ...
        kcolor_random_prob = KColor.random_instance(n_nodes=n_nodes, 
                                                    edge_probability=edge_probability,
                                                    k=k,
                                                    seed=seed)
