...
class TestPortfolioOptimization(unittest.TestCase):
    ...
    def test_portfoliooptimization_terms_weights_constant(self):
        ...
        # w1, w2, po_constant are derived analytically
        w1 = risk_factor*sigma0/2.0 + penalty/2.0
        w2 = (mu0 - risk_factor*sigma0*3)/2.0 + (penalty*budget - penalty*3/2.0)
        po_weights = [w1]*3 + [w2]*3
        po_constant = (risk_factor*sigma0*12.0/4.0 - mu0*3/2.0) + (penalty*12/4.0 - penalty*budget*3 + penalty*budget**2)
        ...
        for w, pw in zip(weights, po_weights):
            self.assertAlmostEqual(w, pw, places=10)
        self.assertEqual(terms, po_terms)
        self.assertAlmostEqual(po_constant, constant, places=10)
