...
class BPSP(Problem):
    ...
    def docplex_bpsp_model(self):
        ...
        w_vars = {
            f"{w}_{i}": mdl.binary_var(name=f"w_{w}_{i}")
        ...
        objective_function = mdl.sum(
            mdl.abs(w_vars[w_keys[i]] - w_vars[w_keys[i + 1]])
    ...
    def qubo(self):
        ...
        return QUBO(
            self.bpsp_graph.number_of_nodes(),
            terms,
            weights,
    ...
    def solve_cplex(self):
        ...
        solution = [
            int(np.round(model.solution.get_value(var)))
            for var in model.iter_binary_vars()
        ]

        # Get the objective value of the solution
        diff_sum = lambda lst: sum(
            abs(lst[i] - lst[i + 1]) for i in range(len(lst) - 1)
        )
    ...
    def plot_paint_sequence(self, paint_sequence, ax=None):
        ...
        fig_width = (
            self.car_sequence.size * 0.8
        )  # This provides each bar approximately 0.8 inch width
        ...
        ax.bar(
            range(len(self.car_sequence)),
            np.ones_like(self.car_sequence),
            color=plot_colors,
            width=1,
            align="center",
        )
        ax.set_xlim(
            -0.5, len(self.car_sequence) - 0.5
        )  # Set x limits to tightly fit bars
        ax.set_ylim(
            0, 1
        )  # Set y limits from 0 to 1 as the bars have a fixed height of 1
