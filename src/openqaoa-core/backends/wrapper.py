...
from ..utilities import exp_val_pair, exp_val_single, negate_counts_dictionary, calculate_calibration_factors
...
class SPAMTwirlingWrapper(BaseWrapper):
    def __init__(self, backend, n_batches):
        ...
        filename = 20230324  # the name of where the calibration data is stored should come externally in the same way as n_batches
    ...
    def get_counts(self, params, n_shots = None):
        ...
        for batch in range(0, self.n_batches):
            ...
            for negated_qubit in negated_qubits:
                ...
                circuit_to_append = self.backend.gate_applicator.apply_gate(negation_gate, negated_qubit, circuit_to_append)
    ...
    def expectation(self, params, n_shots=None) -> float:
        ...
        calibration = False
        if calibration:
            #timestr = time.strftime("%Y%m%d-%H%M%S") # with the time in UTC
            timestr = time.strftime("%Y%m%d")
            with open('calibration_data/{}.json'.format(timestr), 'w') as fp:
                json.dump(counts, fp)
