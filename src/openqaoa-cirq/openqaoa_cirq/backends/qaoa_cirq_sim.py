...
class QAOACirqBackendShotBasedSimulator(
    ...
    def __init__(
        ...
        init_hadamard: bool,
        cvar_alpha: float,
        cirq_simulation_method: str = "automatic",
        ...
        self.backend_simulator = AerSimulator(
            method=cirq_simulation_method.lower(),
    ...
    def qaoa_circuit(self, params: QAOAVariationalBaseParams) -> QuantumCircuit:
        ...
        memory_map = dict(zip(self.cirq_parameter_list, angles_list))
...
class QAOACirqBackendStatevecSimulator(
    ...
    def __init__(
        ...
        self.cirq_cost_hamil_sq = self.cirq_cost_hamil**2
    ...
    def cirq_cost_hamiltonian(self):
        ...
        cirq_cost_hamil = PauliSumOp.from_list(cirq_pauli_op)
    ...
    def parametric_qaoa_circuit(self) -> QuantumCircuit:
        ...
        for each_gate in self.abstract_circuit:
            ...
            if (
                type(each_gate)
                in QAOACirqBackendStatevecSimulator.CIRQ_GATEMAP_LIBRARY
            ):
                decomposition = each_gate.decomposition("trivial")
            else:
                decomposition = each_gate.decomposition("standard")
    ...
    def expectation(self, params: QAOAVariationalBaseParams) -> float:
        ...
        cost = np.real(output_wf.expectation_value(self.cirq_cost_hamil))
        return cost
    ...
    def expectation_w_uncertainty(
        ...
        output_wf = Statevector(ckt)
        self.measurement_outcomes = output_wf.data
        cost = np.real(Statevector(ckt).expectation_value(self.cirq_cost_hamil))
        cost_sq = np.real(Statevector(ckt).expectation_value(self.cirq_cost_hamil_sq))
