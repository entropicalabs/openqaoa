...
class CirqGateApplicator(gates_core.GateApplicator):
    ...
    def gate_selector(self, gate: gates_core.Gate) -> Callable:
        selected_gate = CirqGateApplicator.QISKIT_OQ_GATE_MAPPER[gate.__name__]
    ...
    def apply_1q_rotation_gate(
        ...
        circuit.append(gate(rotation_object.rotation_angle), [qubit_1], [])
        return circuit
    ...
    def apply_2q_rotation_gate(
        ...
        circuit.append(
            gate(rotation_object.rotation_angle), [qubit_1, qubit_2], []
        )
        return circuit

    @staticmethod
    def apply_1q_fixed_gate(
        gate, qubit_1: int, circuit: QuantumCircuit
    ) -> QuantumCircuit:
        circuit.append(gate(), [qubit_1], [])
        return circuit

    @staticmethod
    def apply_2q_fixed_gate(
        gate, qubit_1: int, qubit_2: int, circuit: QuantumCircuit
    ) -> QuantumCircuit:
        circuit.append(gate(), [qubit_1, qubit_2], [])
        return circuit

    def apply_gate(self, gate: gates_core.Gate, *args):
        selected_gate = self.gate_selector(gate)
        if gate.n_qubits == 1:
            if hasattr(gate, "rotation_object"):
                # *args must be of the following format -- (qubit_1,rotation_object,circuit)
                return self.apply_1q_rotation_gate(selected_gate, *args)
            else:
                # *args must be of the following format -- (qubit_1,circuit)
                return self.apply_1q_fixed_gate(selected_gate, *args)
        elif gate.n_qubits == 2:
            if hasattr(gate, "rotation_object"):
                # *args must be of the following format -- (qubit_1,qubit_2,rotation_object,circuit)
                return self.apply_2q_rotation_gate(selected_gate, *args)
            else:
                # *args must be of the following format -- (qubit_1,qubit_2,circuit)
                return self.apply_2q_fixed_gate(selected_gate, *args)
        else:
