...
class QAOACirqQPUBackend(
    ...
    def __init__(
        ...
        if cirq_simulation_method in [0, 1, 2, 3]:
            self.cirq_optimziation_level = cirq_simulation_method
        else:
            raise ValueError(
                f"cirq_simulation_method cannot be {cirq_simulation_method}. Choose between 0 to 3"
            )
    ...
    def qaoa_circuit(self, params: QAOAVariationalBaseParams) -> QuantumCircuit:
        ...
        if self.qaoa_descriptor.routed is True:
            transpiled_circuit = transpile(
                ...
                optimization_level=self.cirq_optimziation_level,
                routing_method="none",
            )
        else:
            transpiled_circuit = transpile(
