...
class TestingQAOAMEBackend(unittest.TestCase):
    ...
    def test_rod(self):
        ...
        me_backend = get_qaoa_backend(circuit_params=circuit_params, device=me_sim, n_shots=10)
        result = me_backend.expectation(variate_params)

        me_backend = get_qaoa_backend(circuit_params=circuit_params, device=me_sim, n_shots=10, noise_model={'decay': False, 'dephasing': False, 'overrot': False, 'spam': False, 'readout01': False, 'readout10': False, 'depol1': gate_error_list[0], 'depol2': gate_error_list[1]}, times=t_gate_list + [t_readout], allowed_jump_qubits=None, target_basis = ['id', 'x', 'sx', 'rz', 'cx'])
        depol_result = me_backend.expectation(variate_params)
        me_backend = get_qaoa_backend(circuit_params=circuit_params, device=me_sim, n_shots=10, noise_model={'decay': T1, 'dephasing': False, 'overrot': False, 'spam': False, 'readout01': False, 'readout10': False, 'depol1': False, 'depol2': False}, times=t_gate_list + [t_readout], allowed_jump_qubits=None, target_basis = ['id', 'x', 'sx', 'rz', 'cx'])
        decay_result = me_backend.expectation(variate_params)
        me_backend = get_qaoa_backend(circuit_params=circuit_params, device=me_sim, n_shots=10, noise_model={'decay': False, 'dephasing': T2, 'overrot': False, 'spam': False, 'readout01': False, 'readout10': False, 'depol1': False, 'depol2': False}, times=t_gate_list + [t_readout], allowed_jump_qubits=None, target_basis = ['id', 'x', 'sx', 'rz', 'cx'])
        dephasing_result = me_backend.expectation(variate_params)
        me_backend = get_qaoa_backend(circuit_params=circuit_params, device=me_sim, n_shots=10, noise_model={'decay': False, 'dephasing': False, 'overrot': False, 'spam': spam_error_prob, 'readout01': False, 'readout10': False, 'depol1': False, 'depol2': False}, times=t_gate_list + [t_readout], allowed_jump_qubits=None, target_basis = ['id', 'x', 'sx', 'rz', 'cx'])
        spam_result = me_backend.expectation(variate_params)
        ...
        readout_result = me_backend.expectation(variate_params)

        overrot_results = list()
        for k in range(1000):
            me_backend = get_qaoa_backend(circuit_params=circuit_params, device=me_sim, n_shots=10, noise_model={'decay': False, 'dephasing': False, 'overrot': overrot_st_dev, 'spam': False, 'readout01': False, 'readout10': False, 'depol1': False, 'depol2': False}, times=t_gate_list + [t_readout], allowed_jump_qubits=None, target_basis = ['id', 'x', 'sx', 'rz', 'cx'])
            overrot_results.append(me_backend.expectation(variate_params))
        overrot_result = np.mean(overrot_results)
