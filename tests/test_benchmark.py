...
class TestingBenchmark(unittest.TestCase):
    ...
    def test_type_qaoa_reference(self):
        ...
        qaoa_analytical.set_device(create_device(name='analytical_simulator', location='local'))
        ...
        assert isinstance(benchmark.reference.backend, type(qaoa_analytical.backend)), "The qaoa reference should have an analytical backend."
    ...
    def test_difference(self):
        ...
        assert benchmark.difference_mean == np.mean(benchmark.difference), "The difference mean should be the mean of the difference matrix."
    ...
    def test_plot_input_checks(self):
        ...
        assert there_is_an_error(benchmark.plot, main=True, reference=False), "An error should be raised when no values are available"
        assert there_is_an_error(benchmark.plot, main=False, reference=True), "An error should be raised when no values are available"
        assert there_is_an_error(benchmark.plot, main=False, reference=False, difference=True), "An error should be raised when no values are available"

        benchmark.run(n_points_axis=4, ranges=[(0,np.pi), (1,)], run_main=True, run_reference=False)
        assert there_is_an_error(benchmark.plot, main=True, reference=True), "An error should be raised when no values are available"
        assert there_is_an_error(benchmark.plot, main=False, reference=True), "An error should be raised when no values are available"
        assert there_is_an_error(benchmark.plot, main=False, reference=False, difference=True), "An error should be raised when no values are available"

        benchmark = QAOABenchmark(qaoa)
        benchmark.run(n_points_axis=4, ranges=[(0,np.pi), (1,)], run_main=False, run_reference=True)
        assert there_is_an_error(benchmark.plot, main=True, reference=True), "An error should be raised when no values are available"
        assert there_is_an_error(benchmark.plot, main=True, reference=False), "An error should be raised when no values are available"
        assert there_is_an_error(benchmark.plot, main=False, reference=False, difference=True), "An error should be raised when no values are available"
