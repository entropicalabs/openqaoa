...
class TestQAOACostBaseClass(unittest.TestCase):
    ...
    def test_gradient_computation(self):
        ...
        for point in test_points:
            ...
            dCdb, dCdg = -4*np.cos(4*beta)*np.sin(2*gamma), -2*np.sin(4*beta)*np.cos(2*gamma)

            for gradient_fun, gradient_name in zip(gradients_fun_list, gradients_types_list):
                dCdb_, dCdg_ = gradient_fun(point)
                assert np.isclose(dCdb, dCdb_, rtol=1e-05, atol=1e-05), f'Gradient computation failed for {gradient_name} on barbell graph. dCdb: {dCdb}, dCdb_: {dCdb_}'
                assert np.isclose(dCdg, dCdg_, rtol=1e-05, atol=1e-05), f'Gradient computation failed for {gradient_name} on barbell graph. dCdg: {dCdg}, dCdg_: {dCdg_}'
    ...
    def test_SPS_sampling(self):
        ...
        backend, params = self.__backend_params(terms=[[0, 1]], weights=[1], p=1, nqubits=2)
        gradient_fun = derivative( backend, params, self.log, 'gradient_w_variance', 
                                    'stoch_param_shift', {'stepsize':0.1, 'n_beta_single':1, 'n_beta_pair':0, 'n_gamma_pair':1, 'n_gamma_single':0})
        ...
        assert n_shots == 4000, f'The number of shots should be {4000} but is {n_shots}.'
